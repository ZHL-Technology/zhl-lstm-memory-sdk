...
- `CHANGELOG.md`
- Git tag, for example `v0.2.3`

While `zhl-memory-core` uses the same Python API across platforms, Ubuntu, Raspberry Pi, and RK3588 use the same project version and Git source. If native platform builds are added later, they should be published from the same source tag and documented in [Platform Guide](PLATFORMS.md).
